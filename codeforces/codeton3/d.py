...
def count(ps, at, rest):
    if rest == 0: return 0
    ret = rest
    for i in range(at, len(ps)):
        if ps[i] > rest: break
        ret -= count(ps, i+1, rest // ps[i])
    return ret

pt = PrimeTable(int(1e5))
def solve(cas):
    N, M = inp()
    a = inp()
    repr = [p for p, b in pt.prime_factorization(a[0])]
    ans = 1
    for i in range(1, N):
        if a[i-1] % a[i]:
            print('0')
        ...
        ps = [p for p in repr if w % p == 0]
        
        ans = norm(ans * count(ps, 0, m))
        # debug(count(ps, 0, m))
        if ans == 0: break
    print(ans)
