...
def dijkstra(s, n, bucket, a):
    ...
    while q:
        u = q.popleft()
        for p, b in pt.prime_factorization(a[u]):
            if vis[p]: continue
            vis[p] = 1
            for x in range(p, MAX, p):
...
def solve(cas):
    n, = inp()
    a = inp()
    s, t = inp(lambda x: int(x)-1)
    
    bucket = Encodict(collections.deque)
    for i, x in enumerate(a):
        bucket[x].append(i)
    
    fa = dijkstra(s, n, bucket, a)
    if fa[t] == -1:
        print(-1)
        return
    ans = [t+1]
